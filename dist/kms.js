...
exports.createKmsSignature = exports.sign = exports.getEthAddressFromKMS = exports.getPublicKey = exports.getEthAddressFromPublicKey = exports.recoverPubKeyFromSig = exports.kms = void 0;
const asn1 = __importStar(require("asn1.js"));
const aws_sdk_1 = require("aws-sdk");
const EthUtil = __importStar(require("ethereumjs-util"));
exports.kms = new aws_sdk_1.KMS({ endpoint: 'kms.us-east-1.amazonaws.com' });
...
const getRS = async (kms, signParams) => {
    const signature = await exports.sign(kms, signParams);
...
const getV = (msg, r, s, expectedEthAddr, eip1559) => {
    let v = 27;
...
exports.getPublicKey = (kms, KeyId) => kms.getPublicKey({ KeyId }).promise();
exports.getEthAddressFromKMS = async (kms, keyId) => {
    const KMSKey = await exports.getPublicKey(kms, keyId);
    if (!KMSKey.PublicKey) {
        throw new Error("Failed to get PublicKey from KMS");
    }
    return exports.getEthAddressFromPublicKey(KMSKey.PublicKey);
};
exports.sign = (kms, signParams) => {
...
exports.createKmsSignature = async (kms, sigParams) => {
    const { keyId, message, address, eip1559 } = sigParams;
    const { r, s } = await getRS(kms, { keyId, message });
    const v = getV(message, r, s, address, eip1559);
    return {
        r: r.toBuffer(),
        s: s.toBuffer(),
        v,
